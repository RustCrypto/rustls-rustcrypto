...
fn local_ping_pong() {
    ...
    let client_thread = thread::spawn(move || {
        ...
        let mut conn = rustls::ClientConnection::new(Arc::new(config), ServerName::try_from("localhost").unwrap()).unwrap();
    ...
    let server_thread = thread::spawn(move || {
        for stream in listener.incoming() {
            match stream {
                Ok(stream) => {
                    let mut ssl_context_build = openssl::ssl::SslContext::builder(openssl::ssl::SslMethod::tls_server()).unwrap();
                    ssl_context_build.set_verify(openssl::ssl::SslVerifyMode::NONE);
                    ssl_context_build.set_ca_file("certs/ca.rsa4096.crt").unwrap();
                    ssl_context_build.set_certificate_file("certs/rustcryp.to.rsa4096.ca_signed.crt", openssl::ssl::SslFiletype::PEM).unwrap();
                    ssl_context_build.set_private_key_file("certs/rustcryp.to.rsa4096.key", openssl::ssl::SslFiletype::PEM).unwrap();
